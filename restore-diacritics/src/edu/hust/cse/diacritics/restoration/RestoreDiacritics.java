...
public class RestoreDiacritics {
  ...
  public static int loadDataBase() {
    ...
    try {
      BufferedReader br =
          new BufferedReader(new InputStreamReader(new FileInputStream(MAP_FILE), "UTF-16"));
      ...
      br.close();
      br = new BufferedReader(new InputStreamReader(new FileInputStream(BIGRAM_FILE), "UTF-16"));
      while ((s = br.readLine()) != null) {
        String temp[] = s.split("\\|");
        if (temp.length == 2)
          htProbability.put(temp[0], Double.valueOf(Double.parseDouble(temp[1])));
      }
      br.close();
    } catch (Exception ex) {
      System.out.print((new StringBuilder("Cannot loadDataBase: ")).append(ex).toString());
    }
    return 1;
  }
    ...
    for (int i = 1; i < wordResult.length; i++) {
      ...
      for (int j = 0; j < temp2.length; j++) {
        ...
        for (int k = 0; k < temp1.length; k++) {
          s =
              new String((new StringBuilder(String.valueOf(temp1[k]))).append(" ").append(temp2[j])
                  .toString());
      ...
      if (!Utils.removeDiacritics(singleWord).equals(word[i])) {
        ...
        for (int j = 0; j < charArrWord.length; j++) {
          ...
          if (word[i].charAt(j) > '@' && word[i].charAt(j) < '[')
            ch = ch.toUpperCase();
          singleWord = (new StringBuilder(String.valueOf(singleWord))).append(ch).toString();
        }

        wordResult[i] = singleWord;
      }
      s = (new StringBuilder(String.valueOf(s))).append(wordResult[i]).append(" ").toString();
    }
    return s.trim();
  }
  ...
  public static String restoreTextSegment(String textSegment) {
    StringTokenizer strTokenizer =
        new StringTokenizer(textSegment, "()[]/-\"\u2026',\u201C\u201D\u2018\u2019?:;", true);
    String tempStr;
    for (textSegment = new String(); strTokenizer.hasMoreTokens(); textSegment =
        (new StringBuilder(String.valueOf(textSegment))).append(tempStr).toString()) {
      boolean check = false;
      tempStr = strTokenizer.nextToken();
      if (tempStr.equals(".") || tempStr.equals(",") || tempStr.equals("?") || tempStr.equals("!")
          || tempStr.equals(":") || tempStr.equals(")") || tempStr.equals("]")
          || tempStr.equals("\u201D") || tempStr.equals("\u2019") || tempStr.equals("\u2026")) {
        check = true;
      }
